...
namespace EasyHealthcare
{
    public partial class doctor : System.Web.UI.Page
    {
        ...
        protected void uploadNote_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand check_PID = new SqlCommand("SELECT COUNT(*) FROM patient WHERE pid=@pID", con);
            check_PID.Parameters.AddWithValue("@pID", pID.Text);
        ...
        protected void View_Click(object sender, EventArgs e)
        {
            ...
            if (userExist != 0)
            {
                // PATIENT INFORMATION
                Label2.Visible = false;
                string getId = "SELECT pid FROM PATIENT WHERE pid=@pID";
                string getFName = "SELECT pFirstname FROM PATIENT WHERE pid=@pID";
                string getLName = "SELECT pLastname FROM PATIENT WHERE  pid=@pID";
                string getAddress = "SELECT pAddress FROM PATIENT WHERE pid=@pID";
                string getPhone = "SELECT pPhone FROM PATIENT WHERE pid=@pID";
                SqlCommand cmd1 = new SqlCommand(getId, con);
                cmd1.Parameters.AddWithValue("@pID", pID.Text);
                ...
                cmd3.Parameters.AddWithValue("@pID", pID.Text);
                SqlCommand cmd4 = new SqlCommand(getAddress, con);
                cmd4.Parameters.AddWithValue("@pID", pID.Text);
                SqlCommand cmd5 = new SqlCommand(getPhone, con);
                cmd5.Parameters.AddWithValue("@pID", pID.Text);
