...
namespace EasyHealthcare
{
    public partial class employee : System.Web.UI.Page
    {
        ...
        protected void discharge_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand check = new SqlCommand("SELECT 1 FROM PATIENT WHERE pid=@disp", con);
            check.Parameters.AddWithValue("@disp", dispID.Text);
            check.Connection = con;
            SqlDataReader reader = check.ExecuteReader();
            if (reader.Read())
            {
                reader.Close();
                SqlCommand dis = new SqlCommand("DELETE FROM patient WHERE pid=@disp", con);
                dis.Parameters.AddWithValue("@disp", dispID.Text);
                dis.ExecuteNonQuery();
        ...
        {
            SqlCommand check_PID = new SqlCommand("SELECT COUNT(*) FROM patient WHERE pid=@IDen", con);
            int patientExist = (int)check_PID.ExecuteScalar();
            check_PID.Parameters.AddWithValue("@IDen", IDentry.Text);
            SqlCommand check_DID = new SqlCommand("SELECT COUNT(*) FROM doctor WHERE did=@IDen", con);
            check_DID.Parameters.AddWithValue("@IDen", IDentry.Text);
